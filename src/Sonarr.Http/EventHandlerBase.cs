...
namespace Sonarr.Http
{
        ...
        where TResource : RestResource, new()
    {
        ...
        protected Task BroadcastResourceChange(ModelAction action)
        {
            ...
            return HubContext?.BroadcastResourceChange<TResource>(action, broadcastName);
        }

        protected Task BroadcastResourceChange(ModelAction action, TResource resource)
        {
            if (GetType().Namespace?.Contains("V3") != true)
                return Task.CompletedTask;

            var broadcastName = this.GetType().GetBroadcastName() ?? resource.GetBroadcastName();
            return HubContext?.BroadcastResourceChange(action, resource, broadcastName);
        }
    }
