...
namespace NzbDrone.Mono.EnvironmentInfo.VersionAdapters
{
    public class MacOsVersionAdapter : IOsVersionAdapter
    {
        ...
        public MacOsVersionAdapter(ILogger<MacOsVersionAdapter> logger, IDiskProvider diskProvider)
            => (_logger, _diskProvider) = (logger, diskProvider);
        ...
        public OsVersionModel Read()
        {
            ...
            {
                _logger.LogDebug("Directory {PListDir} doesn't exist", PLIST_DIR);
                return null;
            }
            ...
            {
                _logger.LogDebug("Couldn't find version plist file in {PListDir}", PLIST_DIR);
                return null;
            }
