...
using Sonarr.Blazor.Shared.Reflection;
            ...
            foreach (var property in GetProperties(type))
            {
                ...
                if (propertyInfo.PropertyType.IsSimpleType())
                {
                    ...
                    var field = new Field
                    {
                        ...
                        Type = fieldAttribute.Type.ToString()[..1].ToLowerInvariant() + fieldAttribute.Type.ToString()[1..], //char.ToLowerInvariant(input.First()) + input.Substring(1)
                        Section = fieldAttribute.Section
                    };

                    if (fieldAttribute.Type == FieldType.Select || fieldAttribute.Type == FieldType.TagSelect)
                    {
                        if (!string.IsNullOrWhiteSpace(fieldAttribute.SelectOptionsProviderAction))
                        {
                    ...
                    {
                        field.Hidden = fieldAttribute.Hidden.ToString()[..1]?.ToLowerInvariant() + fieldAttribute.Hidden.ToString()[1..];
                    }
